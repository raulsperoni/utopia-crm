...
def contact_invoices(request, contact_id):
    ...
    return render(
        request,
        "contact_invoices.html",
        {"contact": contact, "invoice_list": invoice_list, "debt": debt},
    )


def bill_subscription(
    subscription_id, billing_date=None, dpp=10, check_route=False, debug=False
):
    """
    ...
    assert subscription.active, _(
        "This subscription is not active and should not be billed."
    )

    # Check that the subscription is normal
    assert subscription.type == "N", _(
        "This subscription is not normal and should not be billed."
    )

    # Check that the next billing date exists or we need to raise an exception.
    assert subscription.next_billing, _(
        "Could not bill because next billing date does not exist"
    )

    # Check that the subscription has a payment type
    assert subscription.payment_type, _(
    ...
    if subscription.end_date:
        error_msg = _(
            "This subscription has an end date greater than its next billing"
        )
        assert subscription.next_billing < subscription.end_date, error_msg

    if subscription.next_billing > billing_date + timedelta(
        settings.BILLING_EXTRA_DAYS
    ):
    ...
    if not billing_data and not getattr(
        settings, "FORCE_DUMMY_MISSING_BILLING_DATA", False
    ):
        raise Exception(
    ...
    if billing_data and getattr(settings, "REQUIRE_ROUTE_FOR_BILLING", False):
        ...
        elif billing_data["route"] in getattr(
            settings, "EXCLUDE_ROUTES_FROM_BILLING_LIST", []
        ):
            raise Exception(
    ...
    for product_id, copies in list(product_summary.items()):
        ...
        if product.type == "S":
            ...
            item.copies = (
                1  # If the product is a discount, the copies are always 1
            )
    ...
    for discount_product in advanced_discount_list:
        advanced_discount = AdvancedDiscount.objects.get(
            ...
            if product.id in list(product_summary.keys()):
                if product.type == "S":
                    discounted_product_price += (
                        int(product_summary[product.id]) * product.price
                    )
                else:
                    discounted_product_price -= (
                        int(product_summary[product.id]) * product.price
        ...
        if advanced_discount.value_mode == 1:
            ...
            discounted_product_price = round(
                (discounted_product_price * advanced_discount.value) / 100
            )
        item = InvoiceItem()
        frequency_extra = (
            _(" {} months".format(subscription.frequency))
        ...
        item.description = format_lazy(
            "{} {}", discount_product.name, frequency_extra
        )
        item.price = int(
            round(discounted_product_price)
    ...
    if percentage_discount_product:
        ...
        item.description = format_lazy(
            "{} {}", percentage_discount_product.name, frequency_extra
        )
        item.price = int(
            round((subtotal * percentage_discount_product.price) / 100)
        )  # This is to calculate the $
        item.type = "D"
        item.type_dr = 1
    ...
    ).exists() and getattr(settings, "ENVELOPE_PRICE", None):
    ...
    if subscription.frequency > 1:
        ...
        sub_items_amount = sum(
            [item.amount for item in invoice_items if item.type == "I"]
        )
        # Then we make the sum of all discount InvoiceItems
        sub_discounts_amount = sum(
            [item.amount for item in invoice_items if item.type == "D"]
        )
        ...
        frequency_discount_item.description = _(
            "{} months discount ({} discount)".format(
    ...
    if invoice_items:
        try:
            ...
            total = sum(
                [item.amount for item in invoice_items if item.type in "IR"]
            )
            discounts = sum(
                [item.amount for item in invoice_items if item.type == "D"]
            )
            ...
            if subscription.balance and subscription.balance != 0:
                ...
                if subscription.balance > 0:
                    ...
                    balance_item.type = (
                        "D"  # This means the item is a discount.
                    )
                    ...
                    amount -= float(
                        balance_item.amount
                    )  # And then we subtract that value from the invoice
                    ...
                    balance_item.type_dr = 1  # 1 means it's a plain value
                    balance_item.type = (
                        "R"  # This means the item is a surcharge
                    )
                    balance_item.amount = abs(
                        subscription.balance
                    )  # The entire balance is applied to the item
                    amount += float(
                        balance_item.amount
                    )  # And then we add that value to the invoice
            ...
            if amount > 0:
                invoice = Invoice.objects.create(
                    ...
                    + relativedelta(months=subscription.frequency),
                ...
                if getattr(settings, "TEMPORARY_DISCOUNT", None):
                    temporary_discount_list = getattr(
                        settings, "TEMPORARY_DISCOUNT"
                    ).items()
                    for discount_slug, months in temporary_discount_list:
                        if (
                            invoice.has_product(discount_slug)
                            and invoice.subscription.months_in_invoices_with_product(
                                discount_slug
                            )
                            >= months
                        ):
                            invoice.subscription.remove_product(
                                Product.objects.get(slug=discount_slug)
                            )
            ...
            if subscription.balance > 0 and balance_item:
                subscription.balance -= (
                    balance_item.amount
                )  # Then subtract the balance
            if subscription.balance <= 0:
                subscription.balance = (
                    None  # Then if it is zero or less, remove it completely.
                )
            subscription.next_billing = (
                subscription.next_billing or subscription.start_date
            ) + relativedelta(months=subscription.frequency)
            subscription.save()
            ...
            assert amount, _(
                "This subscription wasn't billed since amount is 0"
            )

        except Exception as e:
            raise Exception(
                "Contact {} Subscription {}: {}".format(
                    subscription.contact.id, subscription.id, e
                )
            )
    else:
...
def bill_subscriptions_for_one_contact(request, contact_id):
    ...
    if request.POST.get("creation_date"):
        ...
        for subscription in contact.subscriptions.filter(
            active=True, next_billing__lte=creation_date
        ):
            try:
                invoice = bill_subscription(
                    subscription.id, creation_date, dpp
                )
            except Exception as e:
                ...
                messages.success(
                    request,
                    _(
                        "Invoice {} has been created successfully".format(
                            invoice.id
                        )
                    ),
                )
...
@permission_required(
    ("invoicing.change_invoice", "invoicing.change_creditnote"),
...
def cancel_invoice(request, invoice_id):
    ...
    if not error:
        ...
        if notes:
            messages.error(
                request, _("This invoice already has credit notes.")
            )
        else:
            ...
            i.save()
            messages.success(
                request, _("This invoice was successfully canceled")
            )
    else:
        messages.error(
            request, _("This invoice could not be canceled: {}".format(error))
        )
    return HttpResponseRedirect(
        reverse("admin:invoicing_invoice_change", args=[invoice_id])
    )
...
def download_invoice(request, invoice_id):
    ...
    height = (
        110 * mm
        if getattr(settings, "USE_SQUASHED_SUBSCRIPTION_INVOICEITEMS", False)
        else 140 * mm
    )
    ...
    for page in range(1, 3):
        ...
        c.drawString(
            ...
            _(
                "Issue date: {}".format(
                    invoice.creation_date.strftime("%d/%m/%Y")
                )
            ),
        )
        c.drawString(
        ...
        c.drawString(
            10 * mm, height - 50 * mm, "{}".format(invoice.contact.name)
        )
        c.setFont("Roboto", 5)
        table_data = []
        table_data.append((_("Item"), _("Un."), _("Price"), _("Total")))
        if (
            getattr(settings, "USE_SQUASHED_SUBSCRIPTION_INVOICEITEMS", False)
            ...
            for item in invoice.invoiceitem_set.all():
                table_data.append(
                    [item.description, item.copies, item.price, item.amount]
                )
...
def invoice_filter(request):
    if not request.GET:
        queryset = Invoice.objects.select_related("contact").filter(
